...
3. Copy `.env.example` to `.env` and set `EXPO_PUBLIC_API_URL`.
   - Local testing: use your laptop's LAN IP, for example `http://192.168.0.199:5000`.
   - Public APK: use the deployed backend URL, for example `https://your-buildsphere-api.onrender.com`.
...
5. Set `EXPO_PUBLIC_API_URL` to a real backend URL. For a physical iPhone, do not use `localhost`, `127.0.0.1`, or Android-only `10.0.2.2`; use a public backend URL, your laptop LAN IP, or a backend tunnel.
...
After deployment, open `https://DEPLOYED_BACKEND_URL/health` on a phone browser and confirm it returns `{"status":"ok","service":"BuildSphere API",...}`.

## Public APK Setup
An APK can only be used from anywhere if the backend is also available from anywhere. Do not build a public APK with a `192.168.x.x`, `localhost`, or `127.0.0.1` API URL.
...
4. In `Frontend/.env.production`, set `EXPO_PUBLIC_API_URL=https://DEPLOYED_BACKEND_URL`, `EXPO_PUBLIC_SUPABASE_URL=https://gadhovevmzmzesiqgubb.supabase.co`, and `EXPO_PUBLIC_SUPABASE_ANON_KEY=your-supabase-anon-key`.
...
- Firewall: allow incoming connections on port `5000` for local backend testing.
